...
// Send to just one address at a time to avoid rate limits
for (const addr of ["damisileayoola@gmail.com", "adamsromeo163@gmail.com", "info@brickhealthenergy.org"]) {
  const r = await send([addr], "[PREVIEW] Customer: Order Confirmation #A1B2C3D4", customerHtml(sampleOrder));
